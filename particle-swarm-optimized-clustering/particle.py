...
class Particle:
    def __init__(self,
                 ...
                 w: float = 0.9, 
                 c1: float = 0.5,
                 c2: float = 0.3): # acceleration factors c1=c2=2.025. inertial weight w=0.729
        ...
        self.cluster = self._predict(data).copy()
        self.best_score = quantization_error(self.centroids, self.cluster, data)
        self.velocity = np.zeros_like(self.centroids)
        self._w = w
        self._c1 = c1
        self._c2 = c2

    def update(self, gbest_cluster:np.ndarray, gbest_position: np.ndarray, data: np.ndarray):
    ...
    def _update_centroids(self, data: np.ndarray):
        ...
        predicted = self._predict(data).copy()
        new_score = quantization_error(self.centroids, predicted, data)
        if new_score < self.best_score:
            self.best_score = new_score
            self.best_position = self.centroids.copy()
            self.cluster = predicted.copy() 
